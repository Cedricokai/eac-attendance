...
function Leave() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      // Only filter if we have dates selected
      if (newLeave.startDate && newLeave.endDate) {
        // Filter employees who don't have attendance, overtime, or leave records during the selected period
        const filteredEmployees = employeesData.filter(employee => {
          // Check for attendance conflicts
          const hasAttendance = attendancesData.some(a => 
            a.employee?.id === employee.id && 
            new Date(a.date) >= new Date(newLeave.startDate) && 
            new Date(a.date) <= new Date(newLeave.endDate)
          );
          
          // Check for overtime conflicts
          const hasOvertime = overtimesData.some(o => 
            o.employee?.id === employee.id && 
            new Date(o.date) >= new Date(newLeave.startDate) && 
            new Date(o.date) <= new Date(newLeave.endDate)
          );
          ...
          const hasLeave = leavesData.some(l => 
            ...
            (
              new Date(l.startDate) <= new Date(newLeave.endDate) && 
              new Date(l.endDate) >= new Date(newLeave.startDate)
            )
          );
  ...
  const validateLeave = (leaveId) => {
    setIsValidating(true);
    fetch(`http://localhost:8080/api/leave/validate/${leaveId}`, {
      method: "POST",
    })
      .then((response) => {
        if (!response.ok) {
          throw new Error("Failed to validate leave.");
        }
        return response.json();
      })
      .then((data) => {
        alert("Leave successfully validated!");
        console.log("Validation Response:", data);
        fetchLeaves(); // Refresh leave data
      })
      .catch((error) => {
        alert("Error validating leave. Please try again.");
  ...
  const validateSelectedLeaves = async () => {
    setIsValidating(true);
    setError('');
    
    const leaveIds = isAllSelected 
      ? leaves.map(l => l.id)
      : selectedLeaves;

    if (leaveIds.length === 0) {
      setError("No leaves selected for validation.");
      setIsValidating(false);
      return;
    }

    try {
      const response = await fetch("http://localhost:8080/api/leave/validate/batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ leaveIds })
      });

      if (!response.ok) throw new Error("Failed to validate leaves");
      
      alert(`Successfully validated ${leaveIds.length} leaves`);
      fetchLeaves();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const handleLeaveCheckboxChange = (leaveId, isChecked) => {
    if (isChecked) {
        ...
        <main className="flex-1 max-w-7xl mx-auto px-4 md:px-6 py-6">
          ...
          <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-6 bg-white rounded-xl shadow-sm mt-6">
            <div>
              ...
              <p className="text-gray-600">Track and manage employee leave records</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={validateSelectedLeaves}
                disabled={selectedLeaves.length === 0 && !isAllSelected}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition font-medium text-sm ${
                  (selectedLeaves.length > 0 || isAllSelected)
                  ...
                  {leaves.map((leave) => (
                    <tr key={leave.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input 
                          type="checkbox" 
                          checked={selectedLeaves.includes(leave.id) || isAllSelected}
                          onChange={(e) => handleLeaveCheckboxChange(leave.id, e.target.checked)}
                          className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                        />
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
          ...
          {isCreateMenuOpen && (
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Add Leave</h1>
                <button 
                ...
                {newLeave.startDate && newLeave.endDate && (
                  <div className="bg-blue-50 p-3 rounded-lg">
                    <p className="text-sm text-blue-800">
                      Total leave days: <span className="font-medium">{calculateLeaveDays(newLeave.startDate, newLeave.endDate)}</span>
                    </p>
                  </div>
                )}
