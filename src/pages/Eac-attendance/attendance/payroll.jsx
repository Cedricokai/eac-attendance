...
const fetchPayrollRecords = async (periodId) => {
  try {
    ...
    if (!res.ok) {
      ...
      // The backend now persists temporary allowances, so use the database values directly
...
const generatePayroll = async (periodId) => {
  try {
    ...
    // Prepare temporary allowances in the correct format
    ...
    // Wait a bit for the backend to process, then fetch fresh data
    setTimeout(() => {
...
{selectedPeriod && (
  ...
  {payrollRecords.map((r) => {
    ...
    // Use the database flag instead of frontend state
